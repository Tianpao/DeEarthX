...
import type { TemplateMetadata } from '@/bindings/deearthx/core/template/models';
...
const deletingTemplate = ref<TemplateMetadata | null>(null);
...
function openCreateModal() {
    newTemplate.value = { name: '', version: '1.0.0', description: '', author: '' };
...
function openDeleteModal(template: TemplateMetadata) {
...
async function confirmDelete() {
    ...
    try {
        await TemplateService.Delete(deletingTemplate.value.id);
...
async function openTemplateFolder(template: TemplateMetadata) {
    try {
        await TemplateService.OpenFolder(template.id);
    } catch {
        message.error(t('template.open_folder_failed'));
    }
}

function openEditModal(template: TemplateMetadata) {
...
async function updateTemplate() {
    ...
    try {
        await TemplateService.Update(editingTemplate.value.id, {
            id: editingTemplate.value.id,
            name: newTemplate.value.name,
            version: newTemplate.value.version,
            description: newTemplate.value.description,
            author: newTemplate.value.author,
            created: editingTemplate.value.created,
    ...
    <div class="tw:h-full tw:w-full tw:overflow-y-auto tw:p-6">
                        ...
                        >
                            <div class="tw:flex tw:items-start tw:justify-between tw:gap-3">
                                <div class="tw:min-w-0 tw:flex-1">
                                    <h3 class="tw:truncate tw:text-base tw:font-semibold tw:text-slate-900">{{ template.name }}</h3>
                                    <p class="tw:mt-2 tw:line-clamp-3 tw:text-sm tw:leading-6 tw:text-slate-500">{{ template.description }}</p>
                                </div>
                                <a-tag color="blue">{{ template.version }}</a-tag>
                            </div>
                            ...
                            <div class="tw:mt-5 tw:grid tw:grid-cols-2 tw:gap-3 tw:rounded-lg tw:border tw:border-slate-200 tw:bg-white tw:p-3">
                                <div class="tw:min-w-0">
                                    ...
                                    <div class="tw:truncate tw:text-sm tw:text-slate-700">{{ template.author || '-' }}</div>
                                </div>
                                <div class="tw:min-w-0">
                                    <div class="tw:text-xs tw:text-slate-400">Created</div>
                                    <div class="tw:truncate tw:text-sm tw:text-slate-700">{{ template.created }}</div>
                                </div>
                            </div>
        ...
        >
            <div class="tw:flex tw:items-start tw:gap-3">
                ...
                <div>
                    <p class="tw:mb-2">{{ t('template.delete_confirm', { name: deletingTemplate?.name }) }}</p>
