...
async function getVersionFromJson(): Promise<VersionInfo> {
    try {
        ...
        if (!response.ok) throw new Error(t('about.version_file_read_failed'));
        return await response.json();
    } catch {
        return { version: '1.0.0', buildTime: 'Unknown', author: 'Tianpao' };
    }
}
...
async function fetchSponsors() {
    try {
        ...
        if (list && list.length > 0) {
            sponsors.value = list.map((s: Sponsor) => ({
                ...s,
                id: String(s.id),
...
const sponsorToneClasses: Record<string, string> = {
                    ...
                    >
                        ...
                        <div class="tw:ml-auto tw:flex tw:justify-end">
                            <a-button
                                ...
                                size="middle"
                                class="tw:min-w-[96px]"
                                @click="openUrl(item.bilibiliUrl)"
                            >
